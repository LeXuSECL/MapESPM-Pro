...
% -------------------------------------------------------------------------
% In this toolbox, the Pozzato aging experiment data conducted at SECL are
...
tot_use_cell=1; % This means data from cell #1 will be used for identification.
...
% -------------------------------------------------------------------------
% Because the cell used in the Pozzato experiment is Si+Gr, we have hysteresis.
...
% -------------------------------------------------------------------------
load('../0_Data/NMC_LG/OCV_NMC_LG.mat')
data.OCV=OCV;
...
if cell_num<10
FullVraw=load(['../0_Data/NMC_LG/LG-00',num2str(cell_num),'.mat']);
else
FullVraw=load(['../0_Data/NMC_LG/LG-0',num2str(cell_num),'.mat']);
end
...
% -------------------------------------------------------------------------
% Re-sampling C/20 discharge data
int_num_DC=floor(FullVraw.co20dischargetime(end));
int_time_DC=1:int_num_DC;
int_V_DC=interp1(FullVraw.co20dischargetime',FullVraw.co20dischargeV',int_time_DC);
int_I_DC=interp1(FullVraw.co20dischargetime',FullVraw.co20dischargeI',int_time_DC);

% -------------------------------------------------------------------------
% Using coulomb counting to calculate SOC for C/20 discharge data.
...
% -------------------------------------------------------------------------
% Setting Upper and Lower bounds for 4 θ values
% Parameters 1-4 are for discharge: [θp100, θp0, θn100, θn0]
% -------------------------------------------------------------------------
Initial_position=[0.4, 1, 1, 0.1]; % Initial guess values
Upper_bound=[0.4 1 1 0.4]; % Set upper bound
Lower_bound=[0 0 0 0];         % Set lower bound

% -------------------------------------------------------------------------
% PSO algorithm settings
% -------------------------------------------------------------------------
swarm_size=40;   % Number of particles in PSO
...
% -------------------------------------------------------------------------
% Set the Objective Limit for PSO
% Exit PSO when the optimized cost is lower than this value
% -------------------------------------------------------------------------
% There are 3 profiles, we need to set objective limit for each of them
tot_obj=[0.006,0.006,0.007];
...
clearvars -except  tot_profile_flag tot_obj tot_pso_flag tot_use cell_num tot_err current_use_cell tot_use_cell
% -------------------------------------------------------------------------
% Load the raw data
% -------------------------------------------------------------------------
...
if profile_flag == 0      % C/40 discharge
    soc(1) = 1;
elseif profile_flag == 1  % Nan

elseif profile_flag == 2  % Nan
...
if pso_flag==1
load(['re_iden.mat'])        % This is the defalut EPSM values (Used as initial guess)
elseif pso_flag==2           % This is the previous PSO values (Used as initial guess)
load(['check.mat'])
end
% -------------------------------------------------------------------------

% -------------------------------------------------------------------------
% Always use stoichiometry values obtained from half-cell fitting
% [θp100, θp0, θn100, θn0]
% -------------------------------------------------------------------------
...
Initial_position=[5.09691001300806	5.73706696639855	0.898272519079195	0.786795292160021	12.3331526258388	12.3264603685382	5.09961699108000	7.11507985968810	4.47104221352095	4.71399426766064	-4.06956040523330	-4.12147820449879	-10.2761331941502	1.70039527141064	0.105120196181034	-1.50106066894505	1.50000000000000	1.50000000000000	1.50000000000000];
...
% Here, the following settings mean De is identified using
...
Upper_bound(13)=Initial_position(13);
Lower_bound(13)=Initial_position(13);    
end
...
% Here, the following settings mean Rs is identified using profile 3 (HPPC)
% -------------------------------------------------------------------------
...
% profile 1 (C/40 discharge). Then, when we identify other parameters using
% profile 2 (C/2 discharge) and profile 3 (HPPC), eps_p and eps_n values are fixed.
% -------------------------------------------------------------------------
if profile_flag ==0 || profile_flag ==1
...
save(['../ESPM_Parameters/NMC_LG_cell_',num2str(cell_num),'.mat'],'x_opt')
end
